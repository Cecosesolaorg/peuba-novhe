            ...
            groups.forEach(group => {
                ...
                group.items.forEach((item, index) => {
                    ...
                    if (totalFG > 0 || rcFG > 0) {
                        ...
                        breakdownHtml += `<span class="text-xs font-bold flex items-center justify-end">${textHtml}</span>`;
                    }
                    if (totalMF > 0 || rcMF > 0) {
                        let textHtml = '';
                    ...
                    if (rcOther > 0) {
                        breakdownHtml += `<span class="text-xs font-bold ${hasNote ? 'text-yellow-300' : 'text-orange-500'} flex items-center justify-end">RC:${fmt(rcOther)}</span>`;
                    }
                            ...
                            <div class="flex justify-between items-center p-3 ${hasNote ? 'border-red-400 hover:bg-red-400' : 'border-gray-100 hover:bg-white'} transition cursor-pointer relative">
                                <div class="flex items-center flex-grow py-1" onclick="event.stopPropagation(); window.openModalInStats('${item.id}', '${item.contextKey}', ${item.isProvider ? 'true' : 'false'})"> 
                                    ...
                                    <div class="flex flex-col ml-1">
                                        <span class="text-xs font-bold leading-tight ${textMainClass}">${item.name}</span>
