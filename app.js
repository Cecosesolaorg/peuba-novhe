        ...
        const ACTIVE_VIEW_MODE_STORAGE = 'activeViewMode_V2';
        ...
        let viewMode = localStorage.getItem(ACTIVE_VIEW_MODE_STORAGE) || 'entry'; 
        ...
        function activateContext(selectedKey, selectedText) {
            ...
            // Actualizar vista según el modo guardado
            // Si no hay modo guardado, por defecto es entry
            if (!viewMode) viewMode = 'entry';
            localStorage.setItem(ACTIVE_VIEW_MODE_STORAGE, viewMode);
            ...
            // Forzar actualización de UI de Proveedor si aplica
            if (selectedKey === "PROVEEDOR" && currentSupplier) {
                if (supplierNameDisplay) supplierNameDisplay.textContent = currentSupplier;
        ...
        function selectMenuItem(event) {
            ...
            if (selectedKey === 'STATS_VIEW') {
                viewMode = 'stats';
                localStorage.setItem(ACTIVE_VIEW_MODE_STORAGE, 'stats');
        ...
        function confirmClearData() {
            ...
            localStorage.removeItem(ACTIVE_VIEW_MODE_STORAGE);
        ...
        function renderGlobalStats() {
            ...
            groups.forEach(group => {
                ...
                group.items.forEach((item, index) => {
                    ...
                    // 1. Separar Pesas y RCs, numerando las pesas cronológicamente
                    const pesasArr = [];
                    const rcsBuckets = { FG: [], MF: [], F: [], OTHER: [] };
                    
                    const sortedRegistros = [...item.registros].reverse();
                    let currentPiece = 1;

                    sortedRegistros.forEach(log => {
                        const kg = log.desperdicio_kg;
                        const obs = (log.observation || '').toUpperCase();
                        ...
                        if (isRC) {
                            totalRC += kg;
                            if (destGroup === 'FG') rcFG += kg;
                            else if (destGroup === 'MF') rcMF += kg;
                            else if (destGroup === 'F') rcF += kg;
                            else rcOther += kg;
                            rcsBuckets[destGroup].push({ singleKg, singleAbbr, bgColor });
                        } else {
                            if (destGroup === 'FG') totalFG += kg;
                            else if (destGroup === 'MF') totalMF += kg;
                            else if (destGroup === 'F') totalF += kg;
                            
                            const piecePrefix = `P#${String(currentPiece).padStart(2, '0')} • `;
                            pesasArr.push({ singleKg, singleAbbr, bgColor, piecePrefix, destGroup });
                            currentPiece++;
                        }
                    });

                    // 2. Renderizar emparejando por destino
                    let detailedBreakdownHtml = '<div class="px-3 pb-2 pt-1 grid grid-cols-2 sm:flex sm:flex-wrap gap-1 border-t border-gray-100">';
                    
                    pesasArr.forEach(pesa => {
                        // Columna Izquierda: Pesa Real
                        ...
                        // Columna Derecha: RC del mismo destino (si hay)
                        const matchingRc = rcsBuckets[pesa.destGroup].shift();
                        if (matchingRc) {
                            detailedBreakdownHtml += `<div class="${matchingRc.bgColor} text-white px-1 py-1 rounded text-[11px] font-bold shadow-sm text-center truncate" title="${matchingRc.singleKg} (${matchingRc.singleAbbr})">${matchingRc.singleKg} (${matchingRc.singleAbbr})</div>`;
                        } else {
                            detailedBreakdownHtml += `<div></div>`;
                        }
                    });

                    // 3. Renderizar RCs sobrantes (si los hay)
                    Object.keys(rcsBuckets).forEach(groupKey => {
                        rcsBuckets[groupKey].forEach(rc => {
                            detailedBreakdownHtml += `<div></div>`; // Izquierda vacía
                            detailedBreakdownHtml += `<div class="${rc.bgColor} text-white px-1 py-1 rounded text-[11px] font-bold shadow-sm text-center truncate" title="${rc.singleKg} (${rc.singleAbbr})">${rc.singleKg} (${rc.singleAbbr})</div>`;
                        });
                    });
