...
function updateProductStateFromLocalData() {
    ...
    allProducts = initialProductList.map(product => {
        ...
        const filteredLogs = productLogs.filter(l => {
            const o = (l.observation || '').toUpperCase();
            return !o.includes('PRECARGA') && !o.includes('CARGA TXT');
        });
        return { ...product, id: productId, total_desperdicio_kg: totalKg, expected_kg: expectedKg, registros: filteredLogs.sort((a, b) => b.timestamp - a.timestamp) };
    });
}
...
function formatProductLine(productName, productId, contextKey, logs) {
    logs = logs.filter(log => {
        const o = (log.observation || '').toUpperCase();
        return !o.includes('PRECARGA') && !o.includes('CARGA TXT');
    });
    let totalKg = 0;
    let totalFG = 0;
...
function renderGlobalStats() {
    ...
    contextKeys.forEach((cKey, cIndex) => {
        ...
        initialProductList.forEach(prod => {
            const pId = prod.name.toUpperCase().replace(/\s/g, '_');
            const logs = (contextData[pId] || []).filter(l => {
                const o = (l.observation || '').toUpperCase();
                return !o.includes('PRECARGA') && !o.includes('CARGA TXT');
            });
