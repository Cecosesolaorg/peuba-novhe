        ...
        function updateResponsibleUI() {
            if (currentResponsible && currentResponsible !== 'SIN ASIGNAR') {
                if (responsibleNameDisplay) responsibleNameDisplay.textContent = currentResponsible;
                if (responsibleIndicator) responsibleIndicator.classList.remove('hidden');
            } else {
                if (responsibleNameDisplay) responsibleNameDisplay.textContent = "SIN ASIGNAR";
                if (responsibleIndicator) responsibleIndicator.classList.remove('hidden'); // NUNCA OCULTAR PORQUE AHORA ES EL BOTÓN DE ARRIBA
            }
        }
        ...
        function saveSupplierName() {
            ...
            if (!name) {
                showToast("Por favor ingrese un nombre", "error");
                return;
            }
            ...
            localStorage.setItem(SUPPLIER_SELECTED_KEY, currentSupplier);

            activateContext("PROVEEDOR", "RECEPCIÓN PROVEEDOR");
            closeSupplierNameModal();
            showToast("Proveedor guardado y activo", "success");
        }
        ...
        function activateContext(selectedKey, selectedText) {
            ...
            if (activeMenuItemText) activeMenuItemText.textContent = selectedText;
            viewMode = 'entry';
            ...
            if (selectedKey === "PROVEEDOR") {
                ...
                if (selectedText.includes("RECEPCIÓN PROVEEDOR") && currentSupplier) {
                     ...
                     if (activeMenuItemText) activeMenuItemText.textContent = `PROVEEDOR: ${currentSupplier}`; 
